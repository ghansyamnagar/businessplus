...
import { createProcedureRDRequest, updateProcedureRDRequest } from '@/redux-store/slices/master/master.slice'
import { useDispatch } from 'react-redux'
export type ProcedureRnDType = {
    id: number
    name: string
    description: string
    image?: string
...
type AddEditProcedureRnDInfoProps = {
    open: boolean
    handleClose: (refresh: boolean) => void
    data?: ProcedureRnDType
}
...
const AddEditProcedureRnDInfo = ({ open, handleClose, data }: AddEditProcedureRnDInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
                ...
                updateProcedureRDRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - R & D updated successfully')
            ...
            dispatch(
                createProcedureRDRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - R & D added successfully')
                            handleDialogClose(true)
                        } else {
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Procedure - R & D</div>
            </DialogTitle>
...
export default AddEditProcedureRnDInfo
