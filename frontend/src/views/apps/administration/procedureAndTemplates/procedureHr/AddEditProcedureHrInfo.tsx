...
import { createProcedureHrRequest, updateProcedureHrRequest } from '@/redux-store/slices/master/master.slice'
...
type AddEditProcedureHrInfoProps = {
    open: boolean
    handleClose: (refresh: boolean) => void
    data?: ProcedureHrType
}
...
const AddEditProcedureHrInfo = ({ open, handleClose, data }: AddEditProcedureHrInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
                ...
                updateProcedureHrRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - HR updated successfully')
            ...
            dispatch(
                createProcedureHrRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - HR added successfully')
                            handleDialogClose(true)
                        } else {
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Procedure - HR</div>
            </DialogTitle>
...
export default AddEditProcedureHrInfo
