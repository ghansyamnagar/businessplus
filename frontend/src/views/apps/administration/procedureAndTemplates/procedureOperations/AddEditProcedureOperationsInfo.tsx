...
import { updateProcedureOperationsRequest, createProcedureOperationsRequest } from '@/redux-store/slices/master/master.slice'
...
export type ProcedureOperationsType = {
    id: number
    name: string
    description: string
    image?: string
...
type AddEditProcedureOperationsInfoProps = {
    open: boolean
    handleClose: (refresh: boolean) => void
    data?: ProcedureOperationsType
}
...
const AddEditProcedureOperationsInfo = ({ open, handleClose, data }: AddEditProcedureOperationsInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
    ...
    const onSubmit: SubmitHandler<FormData> = (formData) => {
        ...
        if (data?.id) {
            payload.append('id', String(data.id))
                ...
                updateProcedureOperationsRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - Operations updated successfully')
            ...
            dispatch(
                createProcedureOperationsRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Procedure - Operations added successfully')
                            handleDialogClose(true)
                        } else {
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Procedure - Operations</div>
            </DialogTitle>
...
export default AddEditProcedureOperationsInfo
