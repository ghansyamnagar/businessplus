...
import { createEventRequest, updateEventRequest } from '@/redux-store/slices/master/master.slice'
...
const AddEditEventsInfo = ({ open, handleClose, data }: AddEditEventsInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
    ...
    const onSubmit: SubmitHandler<FormData> = (formData) => {
        ...
        if (imageFile) {
            ...
            const newFileName = `events_${Date.now()}.${extension}`
                ...
                updateEventRequest({
                    ...
                    callback: (res: any, err: any) => {
                        ...
                        if (res?.status === 'success') {
                            toast.success(res?.message || 'Events updated successfully')
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Events</div>
            </DialogTitle>
...
export default AddEditEventsInfo
