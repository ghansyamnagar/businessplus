...
import { getEventListRequest, deleteEventRequest } from '@/redux-store/slices/master/master.slice'
...
// Component Imports
import AddEditEventsInfo from './AddEditEventsInfo'
import type { EventsType } from './AddEditEventsInfo'
...
type EventsTypeWithAction = EventsType & {
    action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<EventsTypeWithAction>()

const EventsListTable = () => {
    // States
    const [addOpen, setAddOpen] = useState(false)
    const [selectedItem, setSelectedItem] = useState<EventsType | undefined>(undefined)
    ...
    const eventsPermission = useModulePermission('Events_photographies')
    ...
    const columns = useMemo<ColumnDef<EventsTypeWithAction, any>[]>(
        () => [
        ...
        [pagination, eventsPermission]
    )

    const table = useReactTable({
        data: data as EventsTypeWithAction[],
        columns,
        filterFns: {
            fuzzy: () => false
        },
        ...
        return (
            <Card>
                ...
                <Box p={5} textAlign='center'>
                    <Typography color='error'>You do not have permission to view Events photography.</Typography>
                </Box>
            </Card>
        )
    }
