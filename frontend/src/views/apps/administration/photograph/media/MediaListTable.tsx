...
import { getMediaListRequest, deleteMediaRequest } from '@/redux-store/slices/master/master.slice'
...
// Component Imports
import AddEditMediaInfo, { type MediaType } from '@/views/apps/administration/photograph/media/AddEditMediaInfo'
...
type MediaTypeWithAction = MediaType & {
    action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<MediaTypeWithAction>()

const MediaListTable = () => {
    // States
    const [addOpen, setAddOpen] = useState(false)
    const [selectedItem, setSelectedItem] = useState<MediaType | undefined>(undefined)
    ...
    const mediaPermission = useModulePermission('Media_photographies')
    ...
    const columns = useMemo<ColumnDef<MediaTypeWithAction, any>[]>(
        () => [
        ...
        [pagination, mediaPermission]
    )

    const table = useReactTable({
        data: data as MediaTypeWithAction[],
        columns,
        filterFns: {
            fuzzy: () => false
        },
    ...
    if (mediaPermission && mediaPermission.acc_view !== 1) {
        return (
            <Card>
                <CardHeader title='Media' className='pbe-4' />
                <Divider />
                <Box p={5} textAlign='center'>
                    <Typography color='error'>You do not have permission to view Media photography.</Typography>
                </Box>
            </Card>
        )
    }
