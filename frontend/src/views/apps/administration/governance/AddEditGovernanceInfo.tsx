...
import { createGovernanceRequest, updateGovernanceRequest } from '@/redux-store/slices/master/master.slice'
...
type AddEditGovernanceInfoProps = {
    open: boolean
...
const AddEditGovernanceInfo = ({ open, handleClose, data }: AddEditGovernanceInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
            ...
            dispatch(
                createGovernanceRequest({
                    ...
                    callback: (response: any) => {
                        ...
                        if (response?.status === 'success') {
                            toast.success('Governance added successfully')
                            handleDialogClose(true)
                        } else {
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Governance</div>
            </DialogTitle>
...
export default AddEditGovernanceInfo
