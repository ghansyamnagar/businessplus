...
import { createLayouRequest, updateLayouRequest } from '@/redux-store/slices/master/master.slice'
...
const AddEditLayoutInfo = ({ open, handleClose, data }: AddEditLayoutInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
    ...
    const onSubmit: SubmitHandler<FormData> = (formData) => {
        ...
        if (data?.id) {
            ...
            if (imageFile) {
                fd.append("file", imageFile)   // 👈 same as Location
            }
                ...
                updateLayouRequest({
                    ...
                    callback: (res: any, err: any) => {
                        ...
                        if (res?.status === 'success') {
                            toast.success(res?.message || 'Layout updated successfully')
            ...
            dispatch(
                createLayouRequest({
                    ...
                    callback: (res: any, err: any) => {
                        ...
                        if (res?.status === 'success') {
                            toast.success(res?.message || 'Layout added successfully')
                            handleDialogClose(true)
                        }
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Layout</div>
            </DialogTitle>
...
export default AddEditLayoutInfo
