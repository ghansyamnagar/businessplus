...
import { createInfrastructureRequest, updateInfrastructureRequest } from '@/redux-store/slices/master/master.slice'
...
type AddEditInfrastructureInfoProps = {
    open: boolean
...
const AddEditInfrastructureInfo = ({ open, handleClose, data }: AddEditInfrastructureInfoProps) => {
    const {
        control,
        reset,
        handleSubmit,
    ...
    const onSubmit: SubmitHandler<FormData> = (formData) => {
        ...
        if (imageFile) {
            ...
            const newFileName = `infrastructure_${Date.now()}.${extension}`
                ...
                updateInfrastructureRequest({
                    ...
                    callback: (res: any, err: any) => {
                        ...
                        if (res?.status === 'success') {
                            toast.success(res?.message || 'Infrastructure updated successfully')
                            handleDialogClose(true)
                        }

                        if (err) {
                            toast.error(err?.message || 'Failed to update infrastructure')
                        }
            ...
            dispatch(
                createInfrastructureRequest({
                    ...
                    callback: (res: any, err: any) => {
                        ...
                        if (res?.status === 'success') {
                            toast.success(res?.message || 'Infrastructure added successfully')
                            handleDialogClose(true)
                        }

                        if (err) {
                            toast.error(err?.message || 'Failed to create infrastructure')
                        }
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Infrastructure</div>
            </DialogTitle>
...
export default AddEditInfrastructureInfo
