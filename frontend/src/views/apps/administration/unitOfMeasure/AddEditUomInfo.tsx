...
// Slice Imports
import { addUomRequest, updateUomRequest } from '@/redux-store/slices/master/master.slice'
...
const schema = object({
    name: pipe(string(), nonEmpty('UOM name is required'), minLength(1, 'UOM name is required'))
})

type FormData = InferInput<typeof schema>

const AddEditUomInfo = ({ open, handleClose, data }: AddEditUomInfoProps) => {
    ...
    const onSubmit: SubmitHandler<FormData> = (formData) => {
        ...
        if (data?.uom_id) {
            ...
            dispatch(updateUomRequest({
                payload: {
                    name: formData.name, // Payload expects 'name'
                    uom_id: data.uom_id
                },
                callback: (response: any, error: any) => {
                    if (response && response.status === 'success') {
                        toast.success(response.message || 'UOM updated successfully')
                        handleDialogClose(true)
                    } else {
                        toast.error(response?.message || 'Error updating UOM')
                    }
                    setLoading(false)
                }
            }))
            ...
            // Add mode
            dispatch(addUomRequest({
                payload: {
                    name: formData.name // Payload expects 'name'
                },
                callback: (response: any, error: any) => {
                    if (response && response.status === 'success') {
                        toast.success(response.message || 'UOM added successfully')
                        handleDialogClose(true)
                    } else {
                        toast.error(response?.message || 'Error adding UOM')
                    }
                    setLoading(false)
                }
            }))
    ...
    return (
        ...
        >
            <DialogTitle variant='h5' className='flex gap-2 flex-col items-center sm:pbs-8 sm:pbe-6 sm:pli-8'>
                <div className='max-sm:is-[80%] max-sm:text-center'>{data ? 'Edit' : 'Add'} Unit of Measure</div>
            </DialogTitle>
