...
// Slice Imports
import { getInitiativesListReq, deleteInitiativeReq } from '@/redux-store/slices/initiatives/initiatives.slice'
...
const columnHelper = createColumnHelper<InitiativeType>()
...
const InitiativesListTable = () => {
    ...
    const [data, setData] = useState<InitiativeType[]>([])
    ...
    const [selectedItem, setSelectedItem] = useState<InitiativeType | undefined>(undefined)
    ...
    const initiativePermission = useModulePermission('Initiatives')
    ...
    const fetchInitiatives = () => {
        ...
        setLoading(true)
        dispatch(getInitiativesListReq({
            payload,
    ...
    const onConfirmDelete = () => {
        if (deleteId) {
            dispatch(deleteInitiativeReq({
                payload: { initiatives_id: deleteId },
                callback: (response: any) => {
                    if (response?.status === 'success') {
            ...
            callback: (res, err) => {
                if (res) {
                    ...
                    link.setAttribute(
                        'download',
                        type === 'pdf'
                            ? 'initiative_report.pdf'
                            : 'initiative_report.xlsx'
                    )
    ...
    const columns = useMemo<ColumnDef<InitiativeType, any>[]>(
        () => [
            ...
            columnHelper.accessor('strategic_objectives_description', {
                header: 'Related Strategic Objectives',
                cell: info => (
                    <div className='truncate max-w-[200px]' title={info.getValue()}>
                        {info.getValue()}
                    </div>
            ...
            columnHelper.accessor('user_id', {
                ...
                cell: info => (
                    <IconButton
                        size='small'
                        onClick={() => {
                            setInitiativeId(info.row.original.initiatives_id)
                            setOpenComment(true)
                        }}
                    >
            ...
            columnHelper.display({
                ...
                cell: ({ row }) => (
                    <div className='flex items-center gap-0.5'>
                        {initiativePermission?.acc_delete === 1 && (
                            <IconButton
                                size='small'
                                onClick={() => handleDelete(row.original.initiatives_id)}
                            >
                                <i className='ri-delete-bin-7-line text-textSecondary' />
                            </IconButton>
                        )}
                        {initiativePermission?.acc_edit === 1 && (
                            <IconButton
                                size='small'
                                onClick={() => {
        ...
        [pagination.pageIndex, pagination.pageSize, initiativePermission]
    )
    ...
    if (initiativePermission && initiativePermission.acc_view !== 1) {
        return (
            <Card>
                <CardHeader title='Initiatives List' className='pbe-4' />
                <Divider />
                <Box p={5} textAlign='center'>
                    <Typography color='error'>You do not have permission to view Initiatives.</Typography>
                </Box>
            </Card>
        )
    }
                ...
                <Grid container spacing={4} className='p-3'>
                    ...
                    <Grid size={{ xs: 12, sm: 12, md: 12 }}>
                        <Box display="flex" justifyContent="flex-end" alignItems="center" gap={3}>
                            ...
                            {initiativePermission?.acc_create === 1 && (
                                <Button
                                    variant='contained'
                                    onClick={() => {
...
export default InitiativesListTable
